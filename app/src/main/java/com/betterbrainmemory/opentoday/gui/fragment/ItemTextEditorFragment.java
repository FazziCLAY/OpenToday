...
import com.betterbrainmemory.opentoday.databinding.FragmentItemTextEditorBinding;
...
public class ItemTextEditorFragment extends Fragment implements BackStackMember, ActivitySettingsMember {
    ...
    private void setupView() {
        ...
        InlineUtil.viewClick(binding.addSystem, () -> {
            putText(binding.editText.getSelectionStart(), "$[]");
            if (!isShowPreview()) {
                setShowPreview(true);
            }
        });
        InlineUtil.viewClick(binding.deleteSystem, this::clearCurrentSystem);

        InlineUtil.viewClick(binding.foregroundColor, () -> new ColorPicker(requireContext(), getSystemColorValue("-"))
        ...
        InlineUtil.viewLong(binding.foregroundColor, () -> setSystemValue("-", "reset"));


        InlineUtil.viewClick(binding.backgroundSystem, () -> new ColorPicker(requireContext(), getSystemColorValue("="))
                .setting(true, true, true)
                .setColorHistoryManager(colorHistoryManager)
                .setColorHistoryMax(20)
        ...
        InlineUtil.viewLong(binding.backgroundSystem, () -> setSystemValue("=", "reset"));

        InlineUtil.viewClick(binding.isBold, () -> {
        ...
        InlineUtil.viewClick(binding.isItalic, () -> {
            String atContent = getSystemStringValue("@", "");
            boolean bold = atContent.contains("bold");
            boolean italic = atContent.contains("italic");
            boolean strikeOut = atContent.contains("~");
        ...
        InlineUtil.viewClick(binding.isStrikeOut, () -> {
            String atContent = getSystemStringValue("@", "");
            boolean bold = atContent.contains("bold");
            boolean italic = atContent.contains("italic");
            boolean strikeOut = atContent.contains("~");
        ...
        for (View view : new View[]{binding.isItalic, binding.isStrikeOut, binding.isBold}) {
            InlineUtil.viewLong(view, () -> setSystemValue("@", "reset"));
        }

        InlineUtil.viewClick(binding.formatTextSize, () -> {
            int size;
            try {
        ...
        InlineUtil.viewLong(binding.formatTextSize, () -> setSystemValue("S", "reset"));
    }
    ...
    private void updateCurrentSystem(int start) {
        ...
        InlineUtil.viewVisible(binding.addSystem, !isSystem(), View.GONE);
        InlineUtil.viewVisible(binding.deleteSystem, isSystem(), View.GONE);

        binding.foregroundColor.setImageTintList(ColorStateList.valueOf(getSystemColorValue("-")));
        InlineUtil.viewVisible(binding.foregroundColor, isSystem(), View.GONE);

        binding.backgroundSystem.setImageTintList(ColorStateList.valueOf(getSystemColorValue("=")));
        InlineUtil.viewVisible(binding.backgroundSystem, isSystem(), View.GONE);


        InlineUtil.viewVisible(binding.isBold, isSystem(), View.GONE);
        binding.isBold.setBackgroundTintList(ColorStateList.valueOf(getStyleTintColor("bold")));


        InlineUtil.viewVisible(binding.isItalic, isSystem(), View.GONE);
        binding.isItalic.setBackgroundTintList(ColorStateList.valueOf(getStyleTintColor("italic")));

        InlineUtil.viewVisible(binding.isStrikeOut, isSystem(), View.GONE);
        binding.isStrikeOut.setBackgroundTintList(ColorStateList.valueOf(getStyleTintColor("~")));

        InlineUtil.viewVisible(binding.formatTextSize, isSystem(), View.GONE);
    }
    ...
    private void setShowPreview(boolean b) {
        this.showPreview = b;
        InlineUtil.viewVisible(binding.formattingPreview, b, View.GONE);
        updatePreview();
