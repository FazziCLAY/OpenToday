...
public class UpdateChecker {
    private static final String V1_URL = "https://fazziclay.github.io/api/project_3/v1/latest/url";
    private static final String V1_BUILD = "https://fazziclay.github.io/api/project_3/v1/latest/build";
    ...
    public static void check(Context context, Result result) {
        if (App.APP_TYPE == AppType.OLD_RED) {
            Log.d("UpdateChecker", "uses v1 api");
            checkV1(context, result);
        } else {
            Log.d("UpdateChecker", "uses v2 api");
            checkV2(context, result);
        }
    }

    private static void checkV2(Context context, Result result) {
        new Thread(() -> {
    ...
    public static void checkV1(Context context, Result result) {
        new Thread(() -> {
            final long CURRENT_TIME = System.currentTimeMillis();
            File cacheFile = new File(context.getExternalCacheDir(), "latest_update_check");
            if (FileUtil.isExist(cacheFile)) {
            ...
            try {
                String stringLatestBuild = NetworkUtil.parseTextPage(V1_BUILD);
                int latestBuild = Integer.parseInt(stringLatestBuild);
                Log.d("UpdateChecker", "latestBuild (remote) = " + latestBuild);

                if (App.VERSION_CODE < latestBuild) {
                    String url = NetworkUtil.parseTextPage(V1_URL);
                    Log.d("UpdateChecker", "url (remote) = " + url);
                    result.run(true, url);
                    Log.d("UpdateChecker", "run. available=true");
                } else {
